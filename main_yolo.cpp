...
private:
	...
	const int inpWidth = 2048;
	const int inpHeight = 2048;
	float confThreshold;
	float nmsThreshold;
	float objThreshold;
			...
			for (i = 0; i < num_grid_y; i++)
			{
				for (j = 0; j < num_grid_x; j++)
				{
					...
					if (box_score > this->objThreshold)
					{
						...
						{
							int ind = k - 5;
							float x = sigmoid_x(pdata[k]) * 8 - 4;
							x = (x * anchor_w + j * this->stride[n])*ratiow;
							landmark[ind] = (int)x;
							float y = sigmoid_x(pdata[k + 1]) * 8 - 4;
							y = (y * anchor_h + i * this->stride[n])*ratioh;
							landmark[ind + 1] = (int)y;
						}
...
int main()
{
	Net_config yolo_nets = {0.3, 0.5, 0.3, "yolov5s"};
